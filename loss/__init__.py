...
class Loss(nn.modules.loss._Loss):
    def __init__(self, args, ckp):
        ...
        if not args.cpu and args.n_GPUs > 1:
            self.loss_module = nn.DataParallel(self.loss_module, device_ids=range(args.n_GPUs))
    ...
    def display_loss(self, batch):
        ...
        for l, c in zip(self.loss, self.log[-1]):
            log.append(f"[{l['type']}: {c / n_samples:.4f}]")
        return ''.join(log)
    ...
    def plot_loss(self, apath, epoch):
        ...
        for i, l in enumerate(self.loss):
            ...
            plt.ylabel('Loss')
            plt.grid(True)
            plt.savefig(f'{apath}/loss_{l["type"]}.pdf')
            plt.close(fig)
    ...
    def load(self, apath, cpu=False):
        kwargs = {'map_location': torch.device('cpu')} if cpu else {}
